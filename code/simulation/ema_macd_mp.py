...
#use libary for multiprocessing
...
def run_ema_macd(ic: InstrumentCollection):
    ...
    #we wont want to overload and slow stuff now so we give limit on how many processes to do at once
    #this will help us to parrell process 4 pairs at once
    limit = 4
    current = 0
    ...
    while current < len(pairs):
        ...
        processes = []
        todo = len(pairs) - current #pairs left to do 
        #keep looping in batches of 4 but when we get to end less than 4 then reduce limit
        if todo < limit:
            limit = todo

        #always be 4 until end
        #_ means we dont plan on using the i value
        for _ in range(limit):
            #send in name of pair and current pair we are working on
